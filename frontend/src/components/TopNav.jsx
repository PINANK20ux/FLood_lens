...
import { Droplets, Plus, Menu, X } from "lucide-react";
...
export default function TopNav() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-ink-800/60 bg-ink-950/95 backdrop-blur-md select-none">
      <div className="mx-auto max-w-7xl px-4 sm:px-8 h-16 flex items-center justify-between">
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.to}
                to={link.to}
                className={`text-xs font-medium tracking-wide transition-colors whitespace-nowrap py-1 ${isActive
        ...
        <div className="hidden lg:flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 rounded-full border border-channel-500/40 text-channel-400 hover:bg-channel-500/10 text-xs font-semibold px-4 py-2 transition-all active:scale-95 shadow-sm"
          >
            <Plus size={14} /> Report Hazard
          </button>
          <button
            onClick={() => navigate("/live")}
            className="rounded-full bg-channel-500 hover:bg-channel-400 text-ink-950 text-xs font-semibold px-4 py-2 transition-all active:scale-95 shadow-sm"
          >
            Open Live Map
          </button>
        </div>
        ...
        <div className="lg:hidden border-t border-ink-800/60 bg-ink-950 px-5 py-4 flex flex-col gap-3 animate-fade-in">
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.to}
                to={link.to}
                className={`text-sm py-1.5 transition-colors ${isActive ? "text-channel-400 font-semibold" : "text-ink-300 hover:text-paper-50"
                  }`}
                onClick={() => setMobileOpen(false)}
              >
                {link.label}
          ...
          <div className="pt-3 border-t border-ink-800/50 flex flex-col gap-2.5">
            ...
            <button
              ...
              className="rounded-full bg-channel-500 text-ink-950 text-xs font-semibold py-2.5 text-center"
            >
              Open Live Map
            </button>
          </div>
